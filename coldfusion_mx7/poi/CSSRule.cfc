...
<cfcomponent
	...
	--->
	<cfset VARIABLES.CSSValidation = StructNew() />
	...
	--->
	<cfset VARIABLES.CSSCache = StructNew() />
	
	
	<!--- Create a struct of valid colors. --->
	<cfset VARIABLES.POIColors = StructNew() />
	<cfset VARIABLES.POIColors.AQUA = true />
	<cfset VARIABLES.POIColors.BLACK = true />
	<cfset VARIABLES.POIColors.BLUE = true />
	<cfset VARIABLES.POIColors.BLUE_GREY = true />
	<cfset VARIABLES.POIColors.BRIGHT_GREEN = true />
	<cfset VARIABLES.POIColors.BROWN = true />
	<cfset VARIABLES.POIColors.CORAL = true />
	<cfset VARIABLES.POIColors.CORNFLOWER_BLUE = true />
	<cfset VARIABLES.POIColors.DARK_BLUE = true />
	<cfset VARIABLES.POIColors.DARK_GREEN = true />
	<cfset VARIABLES.POIColors.DARK_RED = true />
	<cfset VARIABLES.POIColors.DARK_TEAL = true />
	<cfset VARIABLES.POIColors.DARK_YELLOW = true />
	<cfset VARIABLES.POIColors.GOLD = true />
	<cfset VARIABLES.POIColors.GREEN = true />
	<cfset VARIABLES.POIColors.GREY_25_PERCENT = true />
	<cfset VARIABLES.POIColors.GREY_40_PERCENT = true />
	<cfset VARIABLES.POIColors.GREY_50_PERCENT = true />
	<cfset VARIABLES.POIColors.GREY_80_PERCENT = true />
	<cfset VARIABLES.POIColors.INDIGO = true />
	<cfset VARIABLES.POIColors.LAVENDER = true />
	<cfset VARIABLES.POIColors.LEMON_CHIFFON = true />
	<cfset VARIABLES.POIColors.LIGHT_BLUE = true />
	<cfset VARIABLES.POIColors.LIGHT_CORNFLOWER_BLUE = true />
	<cfset VARIABLES.POIColors.LIGHT_GREEN = true />
	<cfset VARIABLES.POIColors.LIGHT_ORANGE = true />
	<cfset VARIABLES.POIColors.LIGHT_TURQUOISE = true />
	<cfset VARIABLES.POIColors.LIGHT_YELLOW = true />
	<cfset VARIABLES.POIColors.LIME = true />
	<cfset VARIABLES.POIColors.MAROON = true />
	<cfset VARIABLES.POIColors.OLIVE_GREEN = true />
	<cfset VARIABLES.POIColors.ORANGE = true />
	<cfset VARIABLES.POIColors.ORCHID = true />
	<cfset VARIABLES.POIColors.PALE_BLUE = true />
	<cfset VARIABLES.POIColors.PINK = true />
	<cfset VARIABLES.POIColors.PLUM = true />
	<cfset VARIABLES.POIColors.RED = true />
	<cfset VARIABLES.POIColors.ROSE = true />
	<cfset VARIABLES.POIColors.ROYAL_BLUE = true />
	<cfset VARIABLES.POIColors.SEA_GREEN = true />
	<cfset VARIABLES.POIColors.SKY_BLUE = true />
	<cfset VARIABLES.POIColors.TAN = true />
	<cfset VARIABLES.POIColors.TEAL = true />
	<cfset VARIABLES.POIColors.TURQUOISE = true />
	<cfset VARIABLES.POIColors.VIOLET = true />
	<cfset VARIABLES.POIColors.WHITE = true />
	<cfset VARIABLES.POIColors.YELLOW = true />
	...
	<cffunction
		...
		<cfif NOT StructKeyExists( VARIABLES.CSSCache, ARGUMENTS.CSS )>
			...
			<cfset LOCAL.CachedPropertyMap = StructNew() />
	...
	<cffunction
		...
		<!--- Grab metric values. --->
		<cfset LOCAL.Values = REMatchArray( "\d+(\.\d+)?(px|em)", ARGUMENTS.Value ) />
		
		<!--- Set up the return array. --->
		<cfset LOCAL.Return = ArrayNew( 1 ) />
		<cfset LOCAL.Return[ 1 ] = "" />
		<cfset LOCAL.Return[ 2 ] = "" />
		<cfset LOCAL.Return[ 3 ] = "" />
	...
	<cffunction
		...
		returntype="array"
		output="false"
		hint="I take a regular expression and a string and return an array of matches.">
		...
		<cfargument 
			...
			type="string" 
			required="true" 
			hint="I am the regular expression being matched."
			/>
		...
		<cfargument 
			...
			type="string" 
			required="true" 
			hint="I am the text in which we are searching."
			/>
			
		<!--- Define the local scope. --->
		<cfset var LOCAL = StructNew() />
		
		<!--- Create a pattern for our search. --->
		<cfset LOCAL.Pattern = CreateObject( "java", "java.util.regex.Pattern" ).Compile( 
			JavaCast( "string", ARGUMENTS.RegEx )
			) />
			
		<!--- Create a pattern matcher based on our text. --->
		...
		<!--- Keep looping over the matches to find values. --->
		<cfloop condition="#LOCAL.Matcher.Find()#">
		
			<!--- Append the match to the array. --->
			<cfset ArrayAppend( LOCAL.Return, LOCAL.Matcher.Group() ) />
		...
		<cfloop
			...
			<cfloop
				...
				<cfif (
					...
					THIS.IsValidValue( LOCAL.Property, LOCAL.Tokens[ LOCAL.Token ] ) 
					)>
			
					<!--- Assign to property. --->
					<cfset LOCAL.CSS[ LOCAL.Property ] = LOCAL.Tokens[ LOCAL.Token ] />
